...
module.exports = class Registrator
{
  ...
  register()
  {
    ...
    {
      console.log('Registrator > Register request in progress...');
...
${this._contact};expires=${this._expires}${this._extraContactParams}`);
      ...
      onReceiveResponse : (response) =>
      {
        ...
        switch (true)
        {
          ...
          case /^2[0-9]{2}$/.test(response.status_code):
            ...
            if (!numContacts)
            {
              console.log('Registrator > no Contact header in response to REGISTER, response ignored');
              break;
            }
            ...
            if (!contact)
            {
              console.log('Registrator > no Contact header pointing to us, response ignored');
              break;
            }
          ...
          case /^423$/.test(response.status_code):
            ...
            { // This response MUST contain a Min-Expires header field
              console.log('Registrator > 423 response received for REGISTER without Min-Expires');
  ...
  unregister(options = {})
  {
    if (!this._registered)
    {
      console.log('Registrator > already unregistered');
