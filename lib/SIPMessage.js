...
class IncomingMessage
{
  ...
  */
  parseHeader(name, idx = 0)
  {
    ...
    if (!this.headers[name])
    {
      console.log(`SIPMessage > header "${name}" not present`);

      return;
    }
    else if (idx >= this.headers[name].length)
    {
      console.log(`SIPMessage > not so many "${name}" headers present`);
    ...
    if (parsed === -1)
    {
      ...
      console.log(`SIPMessage > error parsing "${name}" header field with value "${value}"`);
