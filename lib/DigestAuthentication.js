...
module.exports = class DigestAuthentication
{
  ...
  get(parameter)
  {
    switch (parameter)
    {
      ...
      default:
        console.log('DigestAuthenticaction > get() | cannot get "%s" parameter', parameter);
  ...
  */
  authenticate({ method, ruri, body }, challenge, cnonce = null /* test interface */)
  {
    ...
    if (this._algorithm)
    {
      ...
      {
        console.log('DigestAuthenticaction > authenticate() | challenge with Digest algorithm different than "MD5", authentication aborted');
    ...
    if (!this._nonce)
    {
      console.log('DigestAuthenticaction > authenticate() | challenge without Digest nonce, authentication aborted');
    ...
    if (!this._realm)
    {
      console.log('DigestAuthenticaction > authenticate() | challenge without Digest realm, authentication aborted');
    ...
    if (!this._credentials.password)
    {
      ...
      {
        console.log('DigestAuthenticaction > authenticate() | no plain SIP password nor ha1 provided, authentication aborted');
      ...
      {
        console.log('DigestAuthenticaction > authenticate() | no plain SIP password, and stored `realm` does not match the given `realm`, cannot authenticate [stored:"%s", given:"%s"]', this._credentials.realm, this._realm);
    ...
    if (challenge.qop)
    {
      ...
      else
      {
        ...
        console.log('DigestAuthenticaction > authenticate() | challenge without Digest qop different than "auth" or "auth-int", authentication aborted');
