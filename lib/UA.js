...
module.exports = class UA extends EventEmitter
{
  ...
  {
    console.log('UA > new() [configuration:%o]', configuration);
  ...
  start()
  {
    ...
    {
      console.log('UA > UA is in READY status, not restarted');
    }
    else
    {
      console.log('UA > ERROR: connection is down, Auto-Recovery system is trying to reconnect');
    }
  ...
  {
    console.log('UA > terminateSessions()');
  ...
  stop()
  {
    ...
    {
      console.log('UA > UA already closed');
    ...
    for (const session in this._sessions)
    {
      ...
      {
        console.log(`UA > closing session ${session}`);
  ...
  get(parameter)
  {
    switch (parameter)
    {
      ...
      default:
        console.log('UA > get() | cannot get "%s" parameter in runtime', parameter);
  ...
  set(parameter, value)
  {
    switch (parameter)
    {
      ...
      case 'display_name': {
        ...
        {
          console.log('UA > set() | wrong "display_name"');
      ...
      default:
        console.log('UA > set() | cannot set "%s" parameter in runtime', parameter);
  ...
  receiveRequest(request)
  {
    ...
    {
      console.log('UA > Request-URI does not point to us');
    ...
    if (!request.to_tag)
    {
      switch (method)
      {
        case JsSIP_C.INVITE:
          ...
          {
            console.log('UA > INVITE received but WebRTC is not supported');
            request.reply(488);
          }
          break;
        case JsSIP_C.BYE:
        ...
        case JsSIP_C.CANCEL:
          ...
          {
            console.log('UA > received CANCEL request for a non existent session');
          }
          break;
        case JsSIP_C.ACK:
          /* Absorb it.
    ...
    else
    {
      ...
      else if (method === JsSIP_C.NOTIFY)
      {
        ...
        {
          console.log('UA > received NOTIFY request for a non existent subscription');
  ...
  _loadConfig(configuration)
  {
    ...
    console.log('UA > configuration parameters after validation:');
    for (const parameter in this._configuration)
    {
      ...
      if (Object.prototype.hasOwnProperty.call(config.settings, parameter))
      {
        switch (parameter)
        {
          ...
          case 'registrar_server':
            console.log(`UA > - ${parameter}: ${this._configuration[parameter]}`);
            break;
          case 'password':
          case 'ha1':
            console.log(`UA > - ${parameter}: NOT SHOWN`);
            break;
          default:
            console.log(`UA > - ${parameter}: ${JSON.stringify(this._configuration[parameter])}`);
        }
      }
    }
